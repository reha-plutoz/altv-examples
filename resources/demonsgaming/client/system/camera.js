...
function enabledCamera() {
  ...
  let entityHeight = native.getEntityHeight(
  ...
  let posPlayer = new Vector3(
    alt.Player.local.pos.x,
    alt.Player.local.pos.y,
    alt.Player.local.pos.z + entityHeight
  );
  ...
  cam = new Camera(posPlayer, rotPlayer, 55);
  cam.render();
}
...
function update() {
  if (isEnable) {
    ...
    cam.setRotation(
      new Vector3(
        cam.rotation.x - yMagnitude * 20,
        cam.rotation.y,
        cam.rotation.z - xMagnitude * 20
      )
    );
    native.setEntityCoordsNoOffset(
      alt.Player.local.scriptID,
